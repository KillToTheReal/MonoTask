<template>
    <form method="POST" @submit.prevent="submit">
        ...
        <h4>Color</h4>
        <input type="text" required name="color" v-model="field.color" :id="'color'+(index+1)" class="form-control">
        ...
        <input type="text"  placeholder="Unique" v-model="field.plate_num" required name="plate_num[]" :id="'plate_num'+(index+1)" class="form-control">
         <div class="alert alert-danger" v-if="errors && errors['cars.'+index+'.plate_num'] ">{{errors['cars.' +index+ '.plate_num'][0]}}</div>
        <h4>At parking?</h4>
...
export default {
    ...
    {  
        return{
            ...
            fields:[{'brand':'', 'model':'', 'color':'','plate_num':'','on_parking':''}],
            cars:null,
            success: null,
            errors:{},
            forms:1,
